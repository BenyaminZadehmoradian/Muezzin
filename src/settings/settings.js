...
let timeDisplay, language, adhanFile, bgImage, sunnahTimes, settings, customTimes, jumuahTime,
...
async function saveSettings(){
  ...
  await Promise.all([
    saveTimeDateFormat(),
    window.api.setToStore('latitude', document.getElementById("latInput").value),
    window.api.setToStore('longitude', document.getElementById("lonInput").value),
    window.api.setToStore('locationName', locationName),
    window.api.setToStore("timezone", document.getElementById("tzlist").value),
    window.api.setToStore("language", document.getElementById("langlist").value),
    window.api.setToStore('darkMode', document.getElementById("darkModeCheck").checked),
    window.api.setToStore("timeDisplay.showSeconds", document.getElementById("showSeconds").checked),
    window.api.setToStore("calculationMethod", newCalculationMethod),
    window.api.setToStore("settings", newSettings),
    window.api.setToStore("sunnahTimes", newSunnahTimes),
    saveAdhan(),
    saveBgImage(),
    saveAdjustments(),
    saveCustomSettings(),
...
async function loadSettings(){
  [lat, lon, locationName, language, sunnahTimes, timeDisplay, settings, calculationMethod] = await Promise.all([
    window.api.getFromStore('latitude', 0.00),
    window.api.getFromStore('longitude', 0.00),
    window.api.getFromStore('locationName', ""),
    window.api.getFromStore('language', 'en'),
    window.api.getFromStore("sunnahTimes", { motn: false, totn: false }),
    window.api.getFromStore("timeDisplay", { clockFormat: 12, dateFormat: 'DD/MM/YYYY', showSeconds: true }),
    window.api.getFromStore("settings", { startupSound: false, notifCheck: true, systray: true, adhanCheck: true, autoStart: true, minStart: false }),
    window.api.getFromStore("calculationMethod", { calcMethod: 'MWL', madhab: 'Shafi', hlr: 'TA', pcr: 'CC', shafaq: 'shafaqG' })
  ]);

  const [tzVal, darkMode] = await Promise.all([
    window.api.getFromStore('timezone', 'US/Central'),
    window.api.getFromStore('darkMode', false)
  ]);

  await Promise.all([loadAdhan(), loadCustomSettings(), loadAdjustments(), loadCustomTimes()]);
...
async function returnButton(){
  const set = document.getElementById("return");
  set.onclick= async () => {
    await saveSettings()
...
function setNumberLimit(){
  ...
  let latOldValue = latInput.value;
  latInput.addEventListener("focus", () => {
  ...
  let lonOldValue = lonInput.value;
  lonInput.addEventListener("focus", () => {
...
async function setTZlist(){
  ...
  await fetch('../../ressources/timezones.json')
    .then(res => res.json())
    .then((json) => {
      for (const zone of json){
        for (const timezone of zone["utc"]){
...
async function loadAdhan(){
  ...
  const customCheck = document.getElementById("customAdhan");
  customCheck.checked = adhanFile.adhan.custom
  const duaCheck = document.getElementById("duaCheck");
  duaCheck.checked = adhanFile.dua.enabled;
  const customFajrCheck = document.getElementById("customAdhanFajr");
  customFajrCheck.checked = adhanFile.adhanFajr.custom


  document.getElementById('customAdhanFileButton').onclick = () => {
  ...
  document.getElementById('customAdhanFajrFileButton').onclick = () => {
  ...
  customFajrCheck.addEventListener("change", () => {
    document.getElementById("customAdhanFajrFile").disabled = !customFajrCheck.checked;
    document.getElementById('customAdhanFajrFileButton').disabled = !customFajrCheck.checked;
  })

  document.getElementById("customAdhanFile").addEventListener("change", () => {
    const file = document.getElementById("customAdhanFile").files[0].path;
  ...
  document.getElementById("customAdhanFajrFile").addEventListener("change", () => {
    const file = document.getElementById("customAdhanFajrFile").files[0].path;
...
function disableAdhanListener(){
  ...
  const customAdhanCheck = document.getElementById("customAdhan")
  disableAdhan()
  adhanCheck.addEventListener('change', () => {
    disableAdhan()
  })
  
  customAdhanCheck.addEventListener('change', () => {
    disableAdhan()
  })
...
async function saveAdhan(){
  ...
  if (!customCheck.checked){
    ...
    const file = document.getElementById("customAdhanFile").files
  ...
  if (!customFajrCheck.checked){
    ...
    const file = document.getElementById("customAdhanFajrFile").files
    if (file != undefined && file.length != 0){
...
function addThemeListener(){
  const darkTheme = document.getElementById("darkModeCheck")
...
async function loadBgImage(){
  bgImage = await window.api.getFromStore('bgImage', [true, '../../ressources/images/Qamar_Bani_Hashim_(AS).jpg']);
  const bgImageCheck = document.getElementById("bgImageCheck")
  bgImageCheck.checked  = bgImage[0];
  const file = document.getElementById("customBgImage")
  const fileButton = document.getElementById("customBgImageButton")
  fileButton.onclick = () => {
    file.click();
  };
  ...
  bgImageCheck.addEventListener('change', () => {
    if (!bgImageCheck.checked ){
      file.disabled = true;
      fileButton.disabled = true;
    }
...
async function saveBgImage(){
  const bgImageCheck = document.getElementById("bgImageCheck").checked
  if (!bgImageCheck){
    await window.api.setToStore('bgImage', [false, '../../ressources/images/Qamar_Bani_Hashim_(AS).jpg'])
  }
  else{
    const file = document.getElementById("customBgImage").files
    if (file != undefined && file.length != 0){
      await window.api.setToStore('bgImage', [true, file[0].path])
    }
    else{
      let darkmode = document.getElementById("darkModeCheck").checked
      if (darkmode && bgImage[1] == '../../ressources/images/Qamar_Bani_Hashim_(AS).jpg'){
        await window.api.setToStore('bgImage', [true, '../../ressources/images/Qamar_Bani_Hashim_(AS).jpg'])
      }
      else if (!darkmode && bgImage[1] == '../../ressources/images/Qamar_Bani_Hashim_(AS).jpg'){
        await window.api.setToStore('bgImage', [true, '../../ressources/images/Qamar_Bani_Hashim_(AS).jpg'])
      }
      else{
...
function loadLanguage(lang){
  document.title = `${window.api.getLanguage(lang, "azan")} - ${window.api.getLanguage(lang, "settings")}`;
...
async function loadCustomSettings(){
  const customValues = await window.api.getFromStore('customSettings', [false, 0,0,0]);
  const customCheck = document.getElementById('customCalcCheck');
  const fajrAngle = document.getElementById("fajrAngle");
  const maghribAngle = document.getElementById("maghribAngle");
  const ishaAngle = document.getElementById("ishaAngle");
  
  const delay = await window.api.getFromStore('delay', [false, 0]);
  const delayCheck = document.getElementById('delayCheck');
...
async function loadAdjustments(){
  const adjustements = await window.api.getFromStore('adj', [false, 0,0,0,0,0]);
  const hijriAdj = await window.api.getFromStore('hijriAdj', 0);
  for (let i = 1; i <= 5; i++){
  ...
  document.getElementById("adjCheck").addEventListener("change", () => {
    enableAdjustements(document.getElementById("adjCheck").checked)
  })
...
async function saveAdjustments(){
  const adjCheck = document.getElementById("adjCheck").checked;  
  const fajrAdj = document.getElementById("fajrAdjInput").value;
  const dhuhrAdj = document.getElementById("dhuhrAdjInput").value;
  const asrAdj = document.getElementById("asrAdjInput").value;
  const maghribrAdj = document.getElementById("maghribAdjInput").value;
  const ishaAdj = document.getElementById("ishaAdjInput").value;
  const hijriAdjFinal = document.getElementById("hijriAdjInput").value

  const adjustements = [adjCheck, Math.round(fajrAdj),Math.round(dhuhrAdj),Math.round(asrAdj),Math.round(maghribrAdj),Math.round(ishaAdj)]
...
async function loadQuranSettings(){
  ...
  */
  function loadLanguageList(){
    ...
    .then(json => {
      for (const transLang of json["languages"]){
        const option = document.createElement("option")
    ...
    quranLangListDiv.addEventListener("change", () => {
      hideTranslations()
    })
    diffLangDiv.addEventListener("change", () => {
      if (!diffLangDiv.checked){
        loadDefaultLang()
      }
    })
    document.getElementById("langlist").addEventListener("change",() => {
      if (!diffLangDiv.checked){
        loadDefaultLang()
      }
    })
  ...
  */
  function loadTranslationList(){
    ...
    .catch(err => console.error('Failed to load translation list:', err))
    transListDiv.addEventListener("change", () => {
  ...
  */
  function loadReciterList(){
    ...
    .then(reponse => reponse.json())
    .then(json => {
      for (const reciter of json["recitations"]){
        const option = document.createElement("option")
        option.value = reciter["id"]
        option.innerText = reciter["style"] != null ? `${reciter["reciter_name"]} - ${reciter["style"]}` : reciter["reciter_name"]
    ...
    .catch(err => console.error('Failed to load reciter list:', err))
    reciterList.addEventListener("change", () => {
      quran.recitation.reciter = reciterList.options[reciterList.selectedIndex].value;
    })
  }
  ...
  */
  function quranDisableListeners(){
    showTranslationDiv.addEventListener("change", () => {
    ...
    showTransliterationDiv.addEventListener("change", () => {
...
function setupPreviewButtons() {
  ...
  function startPreview(btn, url) {
    stopPreview(btn);
    previewAudio = new Audio(url);
    previewAudio.volume = 0.8;
    btn.classList.add('playing');
    btn.innerHTML = '<i class="fa fa-stop"></i>';
    previewAudio.play().catch(() => stopPreview(btn));
    previewAudio.addEventListener('ended', () => stopPreview(btn));
  }

  // Adhan preview — resolve relative path to absolute file:// URL
  const adhanBtn = document.getElementById('adhanPreviewBtn');
  if (adhanBtn) {
    adhanBtn.addEventListener('click', () => {
      if (previewAudio && !previewAudio.paused) { stopPreview(adhanBtn); return; }
      const relPath = document.getElementById('adhanList').value;
      const anchor = document.createElement('a');
      anchor.href = relPath;
      startPreview(adhanBtn, anchor.href);
    });
  }

  // Quran reciter preview (Surah Al-Fatiha verse 1)
  const reciterBtn = document.getElementById('reciterPreviewBtn');
  if (reciterBtn) {
    reciterBtn.addEventListener('click', async () => {
      if (previewAudio && !previewAudio.paused) { stopPreview(reciterBtn); return; }
      const reciterId = document.getElementById('reciter').value;
      reciterBtn.disabled = true;
      reciterBtn.innerHTML = '<i class="fa fa-spinner fa-spin"></i>';
      try {
        const res = await fetch(`https://api.quran.com/api/v4/verses/by_key/1:1?audio=${reciterId}&fields=audio`);
        const data = await res.json();
        const audioUrl = 'https://verses.quran.com/' + data.verse.audio.url;
        reciterBtn.disabled = false;
        startPreview(reciterBtn, audioUrl);
      } catch (e) {
        reciterBtn.disabled = false;
        reciterBtn.innerHTML = '<i class="fa fa-play"></i>';
        console.error('Reciter preview failed:', e);
      }
    });
  }
}
...
function loadQueryString(){
  ...
  if (queryString == "?page=quran"){
    const quranTab = new bootstrap.Tab(document.getElementById("v-pills-quran-tab"))
    quranTab.show();
    fromQuran = true;
  }
}
...
async function loadCustomTimes(){
  ...
  disableCustomTimes()
  customTimesCheck.addEventListener("change", () => {
    disableCustomTimes()
  })
  ...
  jumuahCheck.addEventListener("change", () => {
    jumuahInput.disabled = !jumuahCheck.checked;
  })
...
function autocomplete(inp, arr) {
  ...
  function fetchSuggestions(val) {
    ...
    const apiUrl = `https://nominatim.openstreetmap.org/search?format=json&city=${encodeURIComponent(lval)}`;
    fetch(apiUrl)
      .then(r => r.json())
      .then(data => {
        const seen = new Set();
        ...
        unique.slice(0, 20).forEach(item => {
          const b = document.createElement("div");
          const capitalized = lval.charAt(0).toUpperCase() + lval.slice(1);
          let display = item.display_name
            .replace(capitalized, `<b>${capitalized}</b>`)
            .replace(lval, `<b>${lval}</b>`);
          b.innerHTML = display
            + `<input type='hidden' id='name' value='${item.display_name}'>`
            + `<input type='hidden' id='longitude' value='${item.lon}'>`
            + `<input type='hidden' id='latitude' value='${item.lat}'>`;
          b.addEventListener("click", function() {
            inp.value = this.getElementsByTagName("input")[0].value;
            locationName = inp.value;
