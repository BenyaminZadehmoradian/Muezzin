...
function createWindow () {
  ...
  mainWindow = new BrowserWindow({
    ...
    icon: '../../ressources/images/allah.png',
    webPreferences: {
      ...
      submenu: [
        {
          ...
          click: () => { store.clear(); app.relaunch(); app.exit(); }
        },
    ...
    },
    {
      ...
      submenu: [
        ...
        },
        {
          label: 'Reddit',
          click: async () => {
            await shell.openExternal('https://www.reddit.com/r/Azan/')
          }
        },
        {
          label: 'Discord',
...
let lat, lon, calculationMethod, prayerTimes, datePrayerTimes, tomorrowPrayers, adhanPath, adhanSettings, customTimes, jumuahTime;
let customValues, delay;
let settings;
let langFajr, langDhuhr, langAsr, langMaghrib, langIsha, langNow;

let adjustments;
let updateInterval;
let startup = true; // To check if the app just started or not when going back to the main page.
let today = new Date();
...
function getVersion(){
  try {
    ...
    request.on('response', (response) => {
      response.on('data', (chunk) => {
        ...
        try {
          ...
          version = 0;
          console.error(`Couldn't look up Azans version: ${error}`)
        }
    ...
    request.on('close', (error) => {
      if (error != undefined){
        console.error(`Error at end of update check: ${error}`)
      }
    });
    request.setHeader('Content-Type', 'application/json');
    request.end();
  } catch (error) {
    console.error(`Couldn't look for updates: ${error}`)
  }
}
...
function showNotification (message) {
  if (Notification.isSupported()){
    let NOTIFICATION_TITLE = 'Azan'
    let NOTIFICATION_BODY = message
    let NOTIFICATION_ICON = process.platform == "win32" ? path.join(__dirname, '../../ressources/images/icon.ico') : path.join(__dirname, '../../ressources/images/allah.png')
      ...
      if (prayers != undefined && prayers[0] != undefined){
        ...
        if(timeUntilCurrentPrayer[0] == -1 && timeUntilCurrentPrayer[1] == -1 && timeUntilCurrentPrayer[2] == 0){ //-1 because math.floor
          if (settings.adhanCheck){
            mainWindow.webContents.send('playDhikr');
          }
...
async function loadSettings(){
  ...
  adjustments = await store.get('adj', [false, 0,0,0,0,0]);
...
function calcPrayerTimes(date = new Date()){
  ...
  if(adjustments[0]){
    params.adjustments.fajr = adjustments[1]
    params.adjustments.dhuhr = adjustments[2]
    params.adjustments.asr = adjustments[3]
    params.adjustments.maghrib = adjustments[4]
    params.adjustments.isha = adjustments[5]
  }
...
async function setUpHandlers(){
  ...
  ipcMain.handle("loadedUI", () => {
    if (startup) setUpdates()
  })
  ...
  ipcMain.handle("settingsO", () => {})
  ipcMain.handle("settingsC", () => {
    startup = false;
    loadSettings()
  })

  ipcMain.handle("startup-request", () => {
...
// Resolves immediately since calcPrayerTimes is synchronous — kept for API compatibility
function waitFor(something, doSomething){
  // doSomething is an expression already evaluated by the caller, nothing to do here
}
...
function msToTime(duration){ //https://stackoverflow.com/questions/19700283/how-to-convert-time-in-milliseconds-to-hours-min-sec-format-in-javascript
  const seconds = Math.floor((duration / 1000) % 60) + 1,
...
function checkFirstTime(){
  ...
  if (first){
    ...
    const ipgeolocationApi = new IPGeolocationAPI("b9aed80a71d043149013540fb449a384", false); 
  ...
  function loadTheme(){
    if(nativeTheme.shouldUseDarkColors){
      ...
      store.set('bgImage', [true, '../../ressources/images/Qamar_Bani_Hashim_(AS).jpg'])
    }
  }
...
function trayPrayerTimes(){
  const contextMenu = Menu.buildFromTemplate([
    { label: language.loadTrans(lang, 'open'), click: () => mainWindow.show() },
    { type: 'separator' },
    { label: `Fajr: ${changeclockDisplay(prayerTimes.fajr, timeDisplay.clockFormat)}`, click: () => {}},
    ...
    { type: 'separator' },
    { label: language.loadTrans(lang, 'settings'), click: () => { mainWindow.loadFile('src/settings/settings.html'); mainWindow.show(); } },
    { label: language.loadTrans(lang, 'quit'), click: () => { app.isQuiting = true; app.quit(); } }
  ])
  tray.setContextMenu(contextMenu)

  
  function changeclockDisplay(date, timeformat){
    const show0 = n => n < 10 ? `0${n}` : `${n}`
    if (timeformat == 24){
